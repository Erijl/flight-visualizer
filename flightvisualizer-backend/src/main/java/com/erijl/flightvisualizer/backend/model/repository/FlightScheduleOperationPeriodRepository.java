...
public interface FlightScheduleOperationPeriodRepository extends CrudRepository<FlightScheduleOperationPeriod, Integer> {
    ...
    @Query("SELECT new com.erijl.flightvisualizer.backend.model.dto.FlightDateFrequencyDto(f.startDateUtc, COUNT(f)) FROM FlightScheduleOperationPeriod f GROUP BY f.startDateUtc ORDER BY f.startDateUtc ASC")
